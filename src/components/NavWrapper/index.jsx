...
export default function NavWrapper({ isMenuOpen }) {
  ...
  const goToSubMenu = (subMenuName)=>setCurrentSubMenu(subMenuName);
  ...
  return (
    <div className="flex items-start gap-[2vw] mobile:flex-col text-left">
      ...
      <div ref={submenuContainerRef} className="pt-[1vw]">
        {currentMenu === "products" && (
          <>
            ...
            <ProductsMenu
              onBack={() => {goToMenu(null), goToSubMenu(null)}}
             
              onSelectMenu={goToMenu}
              onSelectSubMenu = {goToSubMenu}
            />
          </>
        )}
      ...
      <div ref={subMenuContainer} className="pt-[1vw]">
        {currentSubMenu === "wound-care" && (
          <WoundCareMenu onBack={() => goToMenu("products")} />
        )}
        {currentSubMenu === "endo-surgery" && (
          <EndoSurgeryMenu onBack={() => goToMenu("products")} />
        )}
        {currentSubMenu === "hernia-solutions" && (
          <HerniaSolutionsMenu onBack={() => goToMenu("products")} />
        )}
        {currentSubMenu === "interventional-cardiology" && (
          <InterventionalCardiologyMenu onBack={() => goToMenu("products")} />
        )}
        {currentSubMenu === "cardiac-surgery" && (
...
function MainNav({ onSelectMenu }) {
  return (
    <ul className="space-y-[0.3vw] mobile:space-y-[2vw] border-r pr-[3vw] border-[#d8d8d8] py-5 main-nav tablet:border-none mobile:border-none tablet:pl-[3vw] mobile:pt-[20vw]">
      <li>
        <NavLink
          className="text-[1.5vw] tablet:!text-[4vw] mobile:!text-[6vw]"
          href={"/"}
          linkText="Home"
        />
      </li>
      ...
      <li>
        <button
          ...
          onMouseEnter={() => onSelectMenu("products")}
          className={`text-[1.5vw] fade-in-0 leading-tight text-body tablet:text-[4vw] mobile:text-[6vw] overflow-hidden block relative group aeonik font-light`}
        >
          <span
            data-attr="Products"
          ...
          <ChevronRight className="inline-block w-[1.5vw] tablet:w-[4vw] mobile:w-[5vw] text-current mb-[1%] stroke-[1.2px]" />
        </button>
      </li>
      ...
      <li>
        <NavLink
          ...
          linkText="Research and Development "
        />
      </li>
      <li>
        <NavLink
          ...
          linkText="Advanced MedTech Academy"
        />
      </li>
      <li>
        <NavLink
